...
void __fastcall NewBombExplosions(Player* pBomber, Player* pVictim) {
	...
	auto impulse = dist * (fNewBombExplosionPower * victimCar->fMass / 1000.0 * std::min((GameFlow::fFragDerbyBombMaxDistance - dist.length()) * 2.0 / GameFlow::fFragDerbyBombMaxDistance, 1.0) / std::max(dist.length(), 0.01));
	for (int i = 0; i < 3; i++) {
